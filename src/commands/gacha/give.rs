...
impl Command for GiveCommand {
    fn triggers(&self) -> &[&str] { &["givewaifu"] }
    fn category(&self) -> &str { "gacha" }

    async fn execute(&self, ctx: &CommandContext) -> Result<()> {
        let mentions = extract_mentions(&ctx.body);
        if mentions.is_empty() || ctx.args.len() < 2 {
            return ctx.reply_styled("ꕢ Uso: `#givewaifu @usuario <nombre o ID>`").await;
        }

        let target = &mentions[0];
        if target == &ctx.sender {
            return ctx.reply_styled("ꕢ No puedes darte un personaje a ti mismo.").await;
        }
